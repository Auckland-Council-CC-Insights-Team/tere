...
-   **You don't need to re-invent the wheel every time**: R is built on functions, and these functions can be used multiple times to achieve similar outcomes from different data sources. You can write a single script that will load a data set, tidy it up, produce beautiful charts, and output the files you need. If you need those same charts and files from next year's data, just point your script to that data instead... and you're done!
...
-   **It will only do what you tell it to do**: this comes back to our first point; because you'll be working with data using code when you choose to use R it means it will only perform the operations you tell it to, so no more re-formatted cells.

Best of all, R is completely free! It's also supported by a huge community of users around the world who are more than happy to help.

Oh, and did I mention it was created right here in Tāmaki Makaurau in the 1990s? You have [Ross Ihaka](https://www.teaomaori.news/statistics-legend-ross-ihaka-reflects-on-his-revolutionary-software) (Ngāti Kahungunu, Rangitāne) to thank for that.
...
You've made the first step, but there's one more thing we should install that you'll thank your future self for: Git. This isn't available in Software Centre, but you can download it for free from the [Git for Windows website](https://gitforwindows.org/). Go ahead and click that big ol' download button and keep hitting "Next" until it's done.

# Git? Why do I need that?

Git is a version control system.

Do you find yourself saving spreadsheets as "version 1", "version 2", "final version", "final FINAL version", "final FINAL version edited"? Yeah, you don't need to do that anymore when you use R + Git together. Git will keep track of your files as you change them, so if you need to revert to an earlier version (or just consult an earlier version to understand how you got to where you are with your code), then Git will allow you to do that.
...
You're doing great, and you've got everything you need... for now. Let's get cracking with R by installing your very first package. Fire up R Studio Desktop and in the cons--

# Woah slow down. What the heck is a package?

Oh, sorry. A package is a bunch of code written by someone else to help you do things in R. R is built from packages, so you're going to be using a lot of them. I wrote one myself to help kaimahi get up and running with coding in R. It's called awhina, because it *supports* as you start working in R. In fact, this web page you're looking at was built as part of that package. Pretty cool huh?

# That's very nice, but isn't R good enough on its own without these packages?

When you install R, you're installing what's called Base R or Core R. Once upon a time this was the code written by Ross Ihaka and his co-creator Robert Gentleman at the University of Auckland, but it's evolved a lot since then. Still, there's only so much that base code can do, and it all has to be maintained by a dedicated group of volunteers; it's not in their interests for the base code to do all kinds of things that may only benefit a few people, so that's why we have packages.
...
Ah, yes the console is that window at the bottom left. It looks like this:
...
Good spot! That's right, we're installing devtools first *in order* to install awhina. You see, awhina was only designed for kaimahi at Auckland Council so it'll never be "officially" released. Because of this, we need a special command to install it that's vailable to us via the devtools package... which, as you can probably guess, *does* have an "official" release so we can use `install.packages()` to get that one.

# Installing a package to install a package. Gotcha. Alright I've installed devtools, so how do I get awhina?

That's easy, just pop the following command into the console and hit Enter: `devtools::install_github("lddurbinAC/awhina")`

# Done! Now what?

Well my friend, that's up to you. You've got R, R Studio Desktop, Git for Windows, and two handy packages (devtools and awhina). The next steps are beyond the scope of this little primer, although there's probably one other thing you might wnant to consider...
...
Ah, that might be because your team aren't managing files via SharePoint. You should probably look into that. ICT can help.
...
You're in the right place! Browse around this website and you should find heaps of useful info.
